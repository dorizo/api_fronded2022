...
import { STOCK_ALL_BY_WITEL } from 'services/stock';
...
export default function Index() {
    ...
    const { data, isLoading, refetch } = useQuery('STOCK_ALL_BY_WITEL', STOCK_ALL_BY_WITEL);
    ...
    return (
        <MainCard>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Typography variant="h4">Stok Witel</Typography>
            </div>
            {list &&
                TableComponent(
                    list.map((row, index) => (
