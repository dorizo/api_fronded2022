...
import { ADD_BRAND, DELETE_BRAND, GET_BRANDS, UPDATE_BRAND } from 'services/brand';
...
export default function Index() {
    ...
    const { data, isLoading, refetch } = useQuery('GET_BRANDS', GET_BRANDS);
    ...
    const deleteMutation = useMutation((params) => DELETE_BRAND(params.id), {
    ...
    const onAdd = async (data, callbackSetError) => {
        setLoading(true);
        const response = await ADD_BRAND(data);
    ...
    const onUpdate = async (data, id, callbackSetError) => {
        setLoading(true);
        const response = await UPDATE_BRAND(data, id);
        if (response.status === 400) {
            callbackSetError(response.data.error.form);
        }
        if (response.status === 200) {
    ...
    const onDelete = async () => {
        deleteMutation.mutate({ id: itemSelected.brand_id });
    };
